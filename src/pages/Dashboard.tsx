    ...
    <div className="bg-[#1A1C2C] font-body text-white antialiased min-h-screen">
      ...
      <header className="border-b border-[#2E3050]">
        <div className="max-w-7xl mx-auto px-5 py-8 md:py-12 flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div className="space-y-1.5">
            ...
            <h1 className="text-3xl md:text-5xl font-headline font-extrabold tracking-tight leading-tight text-white">
              Panel de Control
            </h1>
            <p className="text-sm text-[#A0A3B1] max-w-xl leading-relaxed">
              Administra tus ventas, inventario y comisiones en un entorno curado para la excelencia.
            </p>
          </div>
          <Link
            to="/inventario"
            className="flex items-center justify-center w-full md:w-auto flex-shrink-0 gap-2.5 bg-[#7B4CFF] text-white font-bold py-3.5 px-6 rounded-xl hover:bg-[#6B3CEF] active:scale-95 transition-all shadow-lg shadow-[#7B4CFF]/25"
          >
      ...
      <main className="max-w-7xl mx-auto px-5 py-8 md:py-12 space-y-8">
        ...
        <div className="bg-[#20223A] rounded-2xl border border-[#2E3050] overflow-hidden">
          {/* Encabezado */}
          <div className="flex items-center gap-4 px-5 md:px-7 py-5 border-b border-[#2E3050]">
              ...
              <>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1 min-w-0">
                      <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#A0A3B1]" size={17} />
                      <Input
                        type="text"
                        placeholder="Escribe el SKU o nombre..."
                        className="w-full pl-10 pr-4 py-3 bg-[#1A1C2C] border border-[#2E3050] rounded-xl text-white placeholder:text-[#A0A3B1]/50 focus:ring-2 focus:ring-[#7B4CFF] focus:border-transparent outline-none transition-all"
                        value={searchTerm}
                        onChange={(e) => {
                      ...
                      {searchTerm && !productoSeleccionado && resultadosBusqueda.length > 0 && (
                        <div className="absolute z-10 w-full mt-1 bg-[#20223A] border border-[#2E3050] rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                          {resultadosBusqueda.map((item) => (
                            <button
                              key={item.inventario_id}
                              type="button"
                              className="w-full text-left px-4 py-3 hover:bg-[#252840] transition-colors border-b border-[#2E3050] last:border-0 flex justify-between items-center gap-3"
                              onClick={() => {
                            ...
                            >
                              <div className="min-w-0">
                                <p className="font-semibold text-white text-sm truncate">{item.nombre}</p>
                                <p className="text-xs text-[#A0A3B1] mt-0.5">SKU: {item.sku || 'N/A'}</p>
                              </div>
                    ...
                    <button
                      ...
                      className="flex-shrink-0 flex items-center gap-2 px-4 py-3 rounded-xl bg-[#1A1C2C] border border-[#2E3050] text-[#7B4CFF] font-bold hover:border-[#7B4CFF]/50 hover:bg-[#252840] active:scale-95 transition-all"
                    >
                      <QrCode size={20} />
                ...
                <div className="space-y-2.5">
                  ...
                  <input
                    ...
                    className="flex h-12 w-full rounded-xl border border-[#2E3050] bg-[#1A1C2C] px-4 py-2 text-sm text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7B4CFF] focus-visible:border-transparent transition-all"
                  />
                </div>

                {/* Total a cobrar */}
                {productoActual && (
                  <div className="bg-[#FFD600]/10 p-5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center border border-[#FFD600]/25 gap-2">
                    <span className="text-xs font-bold tracking-[0.2em] uppercase text-[#FFD600]/80">Total a cobrar</span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
          {kpis.map((kpi) => {
            ...
            return (
              <div
                key={kpi.id}
                className={`relative bg-[#20223A] rounded-2xl p-4 md:p-5 border overflow-hidden transition-all hover:scale-[1.015] ${
                  isWarning
                    ? 'border-[#FFD600]/30 shadow-[0_0_28px_rgba(255,214,0,0.07)]'
                    : 'border-[#2E3050] hover:border-[#7B4CFF]/30'
                }`}
              >
                ...
                <div className="relative space-y-3">
                  ...
                  <div>
                    <p className="text-xl md:text-2xl font-headline font-extrabold tracking-tight text-white truncate">
                      {kpi.value}
                    </p>
                    <p className="text-[0.7rem] text-[#A0A3B1] mt-0.5 truncate font-medium">{kpi.label}</p>
                  </div>
                  <p className={`text-[0.7rem] font-medium truncate ${
                    isWarning ? 'text-[#FFD600]/70' : 'text-[#A0A3B1]/70'
                  }`}>
                    {kpi.trend}
                  </p>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          {/* Actividad Reciente */}
          <div className="lg:col-span-2 bg-[#20223A] rounded-2xl p-5 md:p-7 border border-[#2E3050] space-y-5">
            <div className="flex items-center justify-between pb-4 border-b border-[#2E3050]">
              <h2 className="text-base md:text-lg font-headline font-bold tracking-tight text-white">Actividad Reciente</h2>
            ...
            <div className="space-y-4">
              {stats.ultimas_ventas && stats.ultimas_ventas.length > 0 ? (
                stats.ultimas_ventas.map((venta) => (
                  <div key={venta.id} className="flex gap-3 items-center justify-between group pb-4 border-b border-[#2E3050] last:border-b-0 last:pb-0">
                    <div className="flex gap-3 items-center flex-1 min-w-0">
                      <div className="w-11 h-11 rounded-xl overflow-hidden bg-[#1A1C2C] border border-[#2E3050] flex-shrink-0 flex items-center justify-center">
                        {venta.imagen ? (
                          <img
                            src={venta.imagen}
                          ...
                          <Package size={18} className="text-[#A0A3B1]/40" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-bold text-white leading-snug group-hover:text-[#7B4CFF] transition-colors truncate">
                          {venta.producto_nombre}
                        </h4>
                        <p className="text-xs text-[#A0A3B1] flex items-center gap-1 mt-0.5">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <p className="text-xs text-[#A0A3B1] flex items-center gap-1 justify-end mt-0.5">
                ...
                <div className="text-center text-[#A0A3B1] py-10">
          ...
          <div className="bg-[#20223A] rounded-2xl p-5 md:p-7 border border-[#2E3050] space-y-5">
            <h3 className="text-base md:text-lg font-headline font-bold tracking-tight text-white">Rendimiento (Mes)</h3>
            ...
            <div className="h-[280px] w-full">
              {stats.grafica_mensual && stats.grafica_mensual.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-[#A0A3B1] space-y-3">
